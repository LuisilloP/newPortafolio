...
const Header = () => {
  ...
  return (
    <div
      className={`shadow-md w-full fixed   md:px-10 px-7 top-0 left-0 z-50 ${
        MouseMoveY == 0 ? 'py-5 bg-neutral-950 ' : ' bg-[#1f2124] py-3'
    ...
    >
      <div className=' flex md:flex items-center justify-between'>
        ...
        <ul
          className={` ${
            MouseMoveY == 0 ? ' bg-neutral-950 ' : ' bg-[#1f2124] '
          } md:flex md:items-center font-special-550 text-special-095 md:pb-0 pb-12 absolute md:static md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 pl-9  ease-out duration-500 ${
        ...
        >
          {urls.map((link, i) => (
            <li key={i} className='md:ml-8 md:my-0 my-7 '>
              <a
                ...
                onClick={() => {
                  document.querySelector(`#${link[0]}`)?.scrollIntoView({ behavior: 'smooth' })
          ...
          <li className='md:ml-8 md:my-0 my-7 hover:text-teal-500 donwload-icon-special  '>
            <a
              ...
              href='https://firebasestorage.googleapis.com/v0/b/portafolioluisdev.appspot.com/o/ArayaLuisCV.pdf?alt=media&token=7b275db8-705f-4b79-b833-396ce041694d'
              rel='noreferrer'
            >
